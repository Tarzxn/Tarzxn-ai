...
**Accounts surviving redeploys** (`data/users.json`) needs real persistent storage, so `render.yaml` mounts a 1 GB disk at `data/` and sets `plan: starter` — persistent disks aren't available on Render's free plan at all, so this is a hard requirement for that guarantee, not optional config. If you'd rather not pay for that, the seed-account env vars (`FORGE_USERNAME`/`FORGE_PASSWORD`) give the exact same "survives every redeploy" guarantee for free, since env vars — unlike disk — are never wiped: Render just recreates that one account from them on every boot. Any *other* accounts created via self-signup still need the disk to survive a redeploy. On startup Forge logs how many accounts it loaded (`[Forge] N account(s) loaded from ...`) — if that's back to 0 after a redeploy where people had signed up, the disk isn't actually attached/persisting and needs checking. Generated files remain intentionally ephemeral either way — pruned after 2 hours regardless of disk.
